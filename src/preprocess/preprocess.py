...
from ..features import KeyFrameFeatures, VideoFeatures
...
def preprocess_video(video_id: str, video_path: Path, output_path: Path, recalc: bool = False):
    ...
    calc_saliency(decoder, features)
    print(f"Calculating CLIP embedding...")
    calc_clip_embedding(decoder, features)
    print(f"Calculating optical flow...")
    calc_optical_flow(decoder, features)
